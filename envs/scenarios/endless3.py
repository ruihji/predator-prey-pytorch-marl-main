...
class Predator(Agent):
    def __init__(self, args):
        ...
        self._obs = deque(maxlen=args.history_len)
        self.obs_range = 1
...
class Scenario(BaseScenario):
    def __init__(self, args):
        ...
        self.n_prey1 = args.n_prey1
        self.n_prey2 = args.n_prey2
        self.map_size = args.map_size
        self.penalty = args.penalty
        self.prey_captured = False

    def make_world(self):
        world = World(width=self.map_size, height=self.map_size)
        ...
        for i in range(self.n_predator):
            agents.append(Predator(self.args))
            self.atype_to_idx["predator"].append(i)

        # add preys
        for i in range(self.n_prey1):
            agents.append(Prey())
            self.atype_to_idx["prey"].append(self.n_predator + i)

        for i in range(self.n_prey2):
            agents.append(Prey2())
            self.atype_to_idx["prey2"].append(self.n_predator + self.n_prey1 + i)
